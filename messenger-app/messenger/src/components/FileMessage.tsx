...
interface FileMessageProps {
    ...
    fileSize: number;
    encryptedData: any;
    onDecryptAndDownload: (encryptedData: any) => Promise<void>;
    onDecryptAndGetData: (encryptedData: any) => Promise<ArrayBuffer>;
    localPreviewUrl?: string; // новый проп
    t: (key: string) => string;
}

export function FileMessage({ fileName, fileType, fileSize, encryptedData, onDecryptAndDownload, onDecryptAndGetData, localPreviewUrl, t }: FileMessageProps) {
    ...
    }, [isImage, encryptedData, fileType, onDecryptAndGetData, localPreviewUrl]);
    ...
    const handleDownload = async () => {
        ...
        try {
            if (localPreviewUrl) {
                ...
                await onDecryptAndDownload(encryptedData);
            }
        } catch (err) {
